...
var DQNAgent = function(env, opt) {
  ...
  this.epsilon = getopt(opt, 'epsilon', 0.1); // for epsilon-greedy policy
  this.alpha = getopt(opt, 'alpha', 0.01); // value function learning rate
  this.tau = getopt(opt, 'tau', .9); //Temperature of weights
...
DQNAgent.prototype = {
  ...
  act: function(slist) {
    ...
    if(Math.random() < this.epsilon) {
      ...
      this.amat = this.forwardQ(this.net, s, false);
      var a = R.maxi(this.amat.w); // returns index of argmax action
    }
  ...
  actRandom: function(slist) {
    ...
    this.amat = this.forwardQ(this.net, s, false);
    var sum = 0;
    for (var i in this.amat["w"]) {
  	  sum += Math.pow(Math.E,this.amat["w"][i]/this.tau);
    }
    for (var i in this.amat["w"]) {
  	  this.amat["w"][i] = (Math.pow(Math.E,this.amat["w"][i]/this.tau)/sum);
    }
    // Determining random action based on probabilies of weights
    var sum=0, r=Math.random(), a=0;
    for (var i in this.amat["w"]) {
      sum += this.amat["w"][i];
      if (r <= sum) {
    	a = i;
